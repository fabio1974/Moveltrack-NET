...
@Named
public class ReadingProtocolHandlerCRX1 extends ReadingProtocolHandler {

	boolean DEBUG_MODE = Constantes.DEBUG_CRX1;
	
	public ReadingProtocolHandlerCRX1()  {
	}
	...
	public void startReading(Socket socket) throws IOException {
		...
		Utils.log(DEBUG_MODE," - START READING PROTOCOL CRX1");
		...
		Utils.log(DEBUG_MODE," - STOP READING PROTOCOL CRX1");
	} 	
	...
	private void transmissionAnalisys(byte[] byteArray,int len, Socket socket) {
		...
		case 0x12:
			...
			saveLocation(loc,ModeloRastreador.CRX1);
			break;
		...
		case 0x16:
			...
			saveLocation(alarmLocation,ModeloRastreador.CRX1);
	...
	private Location getLocationFromLocationPackage(byte[] location) {
		...
		try{
			if(Crc16.isCrcOk(location)){
				...
				Utils.log(DEBUG_MODE,"...getting good CRX1 location package from "+Imei.getImei()+"!");
				loc.setImei(Imei.getImei());
