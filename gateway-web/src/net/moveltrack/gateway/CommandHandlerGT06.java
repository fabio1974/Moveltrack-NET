...
public class CommandHandlerGT06 extends CommandHandler {
	...
	public void sendCommandToTracker(Sms sms) {
		...
		switch (sms.getTipo()) {
		case BLOQUEIO:
			command = buildGT06Command(COMMAND_CUT_OIL,serial1,serial2);
			Utils.log(DEBUG_MODE,"Comando de Bloqueio");
			printBuff(DEBUG_MODE,command);
			break;
		case DESBLOQUEIO:
			command = buildGT06Command(COMMAND_CONNECT_OIL,serial1,serial2);
	...
	public static byte[] buildGT06Command(byte[] commandContent,byte ser1,byte ser2) {
