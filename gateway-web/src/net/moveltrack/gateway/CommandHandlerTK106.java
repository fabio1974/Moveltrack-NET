...
public class CommandHandlerTK106 extends CommandHandler {
	...
	public void sendCommandToTracker(Sms sms) {
		...
		switch (sms.getTipo()) {
		case BLOQUEIO:
			command = buildTK106Command(COMMAND_CUT_OIL,serial1,serial2);
			Utils.log(DEBUG_MODE,"Comando de Bloqueio");
			printBuff(DEBUG_MODE,command);
			break;
		case DESBLOQUEIO:
			command = buildTK106Command(COMMAND_CONNECT_OIL,serial1,serial2);
	...
	public static byte[] buildTK106Command(byte[] commandContent,byte ser1,byte ser2) {
