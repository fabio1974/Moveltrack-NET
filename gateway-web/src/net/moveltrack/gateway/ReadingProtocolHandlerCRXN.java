...
@Named
public class ReadingProtocolHandlerCRXN extends ReadingProtocolHandler {

	boolean DEBUG_MODE = Constantes.DEBUG_CRXN;
	
	public ReadingProtocolHandlerCRXN()  {
	}
	...
	public void startReading(Socket socket) throws IOException {
		...
		Utils.log(DEBUG_MODE," - START READING PROTOCOL CRXN");
		...
		Utils.log(DEBUG_MODE," - STOP READING PROTOCOL CRXN");
	} 	
	...
	private void transmissionAnalisys(byte[] byteArray,int len, Socket socket) {
		...
		}else if(byteArray[0]==0x79){
			
			switch (byteArray[4]) {
			case (byte)0x94:
				//Utils.logSemData(DEBUG_MODE," INFORMATION TRANSMISSION from "+Imei.getImei()+"->");
				//printBuff(true,byteArray);
	...
	private void extractInformation(byte[] byteArray) {
	...
	private void heartBeatResponse(byte[] heartBeat,Socket socket) {
			if(Crc16.isCrcOk(heartBeat)){
				...
				byte[] commandLocation = CommandHandlerCRXN.buildCRXNCommand(CommandHandlerCRXN.COMMAND_LOCATION,Serial1.getSerial1(),Serial2.getSerial2());
	...
	private void sendCommandLocation(Socket socket){
		byte[] commandLocation = CommandHandlerCRXN.buildCRXNCommand(CommandHandlerCRXN.COMMAND_LOCATION,Serial1.getSerial1(),Serial2.getSerial2());
	...
	private void receivedCommandAnalisys(String receivedCommand) {
		Utils.log(DEBUG_MODE,"Conteudo do comando: "+receivedCommand);
	...
	private Location getLocationFromLocationPackage(byte[] location) {
		...
		try{
			if(Crc16.isCrcOk(location)){
				...
				Utils.log(DEBUG_MODE,"...getting good CRXN location package from "+Imei.getImei()+"!");
				loc.setImei(Imei.getImei());
	...
	public static void main(String[] args) {
		ReadingProtocolHandlerCRXN p = new ReadingProtocolHandlerCRXN();
