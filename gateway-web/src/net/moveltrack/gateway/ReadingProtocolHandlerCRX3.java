...
@Named
public class ReadingProtocolHandlerCRX3 extends ReadingProtocolHandler {

	boolean DEBUG_MODE = Constantes.DEBUG_CRX3;
	
	public ReadingProtocolHandlerCRX3()  {
	}
	...
	public void startReading(Socket socket) throws IOException {
		...
		Utils.log(DEBUG_MODE," - START READING PROTOCOL CRX3");
		...
		Utils.log(DEBUG_MODE," - STOP READING PROTOCOL CRX3");
	} 	
	...
	private void transmissionAnalisys(byte[] byteArray,int len, Socket socket) {
		...
		case 0x12:
			...
			saveLocation(loc,ModeloRastreador.CRX3);
			break;
		...
		case 0x16:
			...
			saveLocation(alarmLocation,ModeloRastreador.CRX3);
	...
	private void heartBeatResponse(byte[] heartBeat,Socket socket) {
			if(Crc16.isCrcOk(heartBeat)){
				...
				byte[] commandLocation = CommandHandlerCRX3.buildCommand(CommandHandlerCRX3.COMMAND_LOCATION,Serial1.getSerial1(),Serial2.getSerial2());
	...
	private Location getLocationFromLocationPackage(byte[] location) {
		...
		try{
			if(Crc16.isCrcOk(location)){
				...
				Utils.log(DEBUG_MODE,"...getting good CRX3 location package from "+Imei.getImei()+"!");
				loc.setImei(Imei.getImei());
	...
	public static void main(String[] args) {
		ReadingProtocolHandlerCRX3 p = new ReadingProtocolHandlerCRX3();
