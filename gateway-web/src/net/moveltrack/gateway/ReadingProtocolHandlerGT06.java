...
public class ReadingProtocolHandlerGT06 extends ReadingProtocolHandler {


	boolean DEBUG_MODE = Constantes.DEBUG_GT06;

	
	public ReadingProtocolHandlerGT06()  {
	}
	...
	public void startReading(Socket socket) throws IOException {
		...
		Utils.log(DEBUG_MODE," - START READING PROTOCOL GT06");
		...
		Utils.log(DEBUG_MODE," - STOP READING PROTOCOL GT06");
	} 	
	...
	private void transmissionAnalisys(byte[] byteArray,int len, Socket socket) {
		...
		case 0x12:
			...
			saveLocation(loc,ModeloRastreador.GT06);
			break;
	...
	private void heartBeatResponse(byte[] heartBeat,Socket socket) {
			if(Crc16.isCrcOk(heartBeat)){
				...
				byte[] commandLocation = CommandHandlerGT06.buildGT06Command(CommandHandlerGT06.COMMAND_LOCATION,Serial1.getSerial1(),Serial2.getSerial2());
	...
	private Location getLocationFromLocationPackage(byte[] location) {
		...
		try{
			if(Crc16.isCrcOk(location)){
				...
				Utils.log(DEBUG_MODE,"...getting good GT06 location package from "+Imei.getImei()+"!");
				loc.setImei(Imei.getImei());
	...
	public static void main(String[] args) {
		//ProtocolHandlerCRX1 p = new ProtocolHandlerCRX1();
		...
		//Location loc = p.getHeartBeatStatus(hbeat);
	}	
