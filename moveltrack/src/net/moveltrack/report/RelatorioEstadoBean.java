...
@Named
@ViewScoped
public class RelatorioEstadoBean extends Report implements Serializable {
	...
	protected void setJRBeanCollectionDataSource() {
		...
		if(loginBean.getPessoaLogada() instanceof Cliente) {
			...
			list = reportDao.getRelatorioEstado(cliente,grb.getInicio(),grb.getFim());
		}else
			list = reportDao.getRelatorioEstado(grb.getInicio(),grb.getFim());
	...
	protected void setTotalParameters() {
		...
		for (RelatorioFrota relatorioFrota : list) {
			...
			LitrosValor lv = reportDao.getDespesasLitroPorEstado(relatorioFrota.getId(),grb.getInicio(),grb.getFim());
